...
local startTime = nil
local user = {}
local themesongFilename = 'lib/1.mp3'
local gameOverFilename = 'lib/bits_game_over.mp3'
local gameWinFilename = 'lib/bits_game_win.mp3'
...
function love.load()
  ...
  if TS ~= nil then
    themesongFilename = "lib/"..TS..".mp3"
    -- TODO: error handling on bad inputs to TS
  end

  for i=1,9 do
    table.insert(soundFilenames, 'lib/fx_'..i..'.mp3')
  end
...
function PLAYSND(filename, volume, shouldLoop)
  if type(filename) == 'number' then
    filename = 'lib/fx_'..tostring(filename)..'.mp3'
  end
  if volume == nil then
    volume = 1.0
  end
